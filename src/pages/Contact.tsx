    ...
    <div className="pt-24 pb-16">
      <div className="container mx-auto px-4">
        <FadeUp className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">Have questions? Need a bulk quote? We're here to help.</p>
        </FadeUp>

        <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {infoCards.map(card => (
            <StaggerItem key={card.title}>
              <motion.div whileHover={{ y: -4 }} className="bg-card border border-border rounded-xl p-5 text-center">
          ...
          <FadeUp>
            <div className="bg-card border border-border rounded-2xl p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Textarea placeholder="Your message or requirements..." rows={5} required value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} />
                <Button type="submit" className="w-full bg-solar-gradient text-primary-foreground hover:opacity-90 h-12" size="lg">
